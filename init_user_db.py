...
# DATABASE = "forum.db"
# Lấy đường dẫn tuyệt đối đến thư mục chứa file init_user_db.py này
_basedir = os.path.abspath(os.path.dirname(__file__))
DATABASE = os.path.join(_basedir, "user.db")
...
def init_db():
    # NỘI DUNG MỚI 
    models_path = os.path.join(_basedir, "user_model.sql")
...
def get_all_forum_posts():
    ...
    db = get_db()
    cursor = db.execute("""
        SELECT p.id, p.title, p.content, p.tag, p.created_at, p.questioner_id, u.username
        FROM posts p
        JOIN users u ON p.questioner_id = u.id
    """)
    ...
    for row in posts:
        ...
        ans_cursor = db.execute("""
            ...
            WHERE a.post_id = ?
        """, (row["id"],))
        answers = [{"content": a["content"], "answerer_username": a["answerer_username"]} for a in ans_cursor.fetchall()]
